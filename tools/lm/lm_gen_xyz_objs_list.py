...
class XyzGen(object):
    def __init__(self, objs_list, variant):
        self.renderer = None
        self.objs_list = objs_list
        self.variant = variant

        self.data_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/" + variant + "/train_pbr"))
        self.model_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/" + variant + "/models"))
        self.model_paths = [osp.join(self.model_dir, f"obj_{cls_idx:06d}.ply") for cls_idx in self.objs_list]
        self.xyz_root = osp.normpath(osp.join(self.data_dir, "xyz_crop"))
    ...
    def main(self):
        for scene in tqdm(scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                im_path = osp.join(self.data_dir, scene, f"rgb/{int_im_id:06d}.jpg")
                annos = gt_dict[str_im_id]

                for anno_i, anno in enumerate(annos):
                    obj_id = anno["obj_id"]
                    if obj_id in self.objs_list:
                        # read Pose and K
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="gen lmo random texture pbr xyz")
    parser.add_argument("--gpu", type=str, default="0", help="gpu")
    parser.add_argument("--vis", default=False, action="store_true", help="vis")
    parser.add_argument('--variant', type=str, default="lmo", help="lm, lm_3, lm_5r, etc.")
    ...
    xyz_gen = XyzGen(objs_list, args.variant)
    xyz_gen.main()
