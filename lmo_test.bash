...
do
    ...
    do
        ...
        OUTPUT_FILE="$SCRIPT_DIR/configs/gdrn/${variant[@]}/a6_cPnP_${variant[@]}_${lmo_object}.py"

        # Replace "ape" with the current object name in the input file and save to the output file
        sed -e "s/OBJ/${lmo_object}/g" -e "s/VARIANT/${variant}/g" "$INPUT_FILE" > "$OUTPUT_FILE"
    done
done
