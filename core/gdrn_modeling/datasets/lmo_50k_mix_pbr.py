...
class LM_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo_50k_mix/train_pbr"))
...
SPLITS_LM_PBR = dict(
    lmo_50k_mix_pbr_train=dict(
        name="lmo_50k_mix_pbr_train",
    ...
    lm_50k_mix_pbr_train=dict(
        name="lm_50k_mix_pbr_train",
...
for obj in ref.lmo_full.objects:
    for split in ["train"]:
        name = "lmo_50k_mix_pbr_{}_{}".format(obj, split)
        if split in ["train"]:
            filter_invalid = True
        else:
