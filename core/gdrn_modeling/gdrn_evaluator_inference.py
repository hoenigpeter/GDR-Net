...
def gdrn_inference_on_dataset(cfg, model, data_loader, evaluator, amp_test=False):
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            print("roi_img: ", batch["roi_img"])
            print("roi_classes: ", batch["roi_cls"])
            print("roi_cam: ", batch["roi_cam"])
            print("roi_whs: ", batch["roi_wh"])
            print("roi_centers: ", batch["roi_center"])
            print("resize_ratios: ", batch["resize_ratio"])
            print("roi_coord_2d: ", batch.get("roi_coord_2d", None))
            print("roi_extents: ", batch.get("roi_extent", None))
            print()
